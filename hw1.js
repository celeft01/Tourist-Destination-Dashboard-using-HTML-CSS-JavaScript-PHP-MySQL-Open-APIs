...
function databaseInsert(address, region, city){
    ...
    data1.username="celeft01";
    data1.address=address;
    data1.region=region;
    data1.city=city;
	data1.country="Cyprus";

    xhr.send(JSON.stringify(data1));

}

document.getElementById("btn3").addEventListener("click", function(){databaseGet()});
...
function databaseGet(){
    ...
    if (xhr.status >= 200 && xhr.status < 300) {
		console.log(xhr.responseText);
        logInfo=JSON.parse(xhr.responseText);
        for (let i=0; i<5; i++) {
            var date = new Date(logInfo[i].timestamp * 1000);
            var fullDate = date.getDate() + " " + montharr[date.getMonth()] + " " + date.getFullYear() + " " + date.getHours() + ":" + date.getMinutes();
            document.getElementById("mtime"+(i+1)).innerText=fullDate;
            document.getElementById("maddress"+(i+1)).innerText=logInfo[i].address;
            document.getElementById("mregion"+(i+1)).innerText=logInfo[i].region;
            document.getElementById("mcity"+(i+1)).innerText=logInfo[i].city;
	...
	const username='celeft01';
    
    xhr.open('GET', 'action_page.php?username='+username);
    xhr.send();
}
...
function NominaF(address, region, city){
    ...
    if (xhr.status >= 200 && xhr.status < 300) {
        ...
        if(xhr.responseText!=='[]'){
            ...
            databaseInsert(address, region, city);
...
function attractionsF(city){
    ...
    if (xhr.status >= 200 && xhr.status < 300) {
        ...
        // t4=xhr.responseText;
        var answer=t4.choices[0].text.trim();
        console.log(answer);
        ...
        document.getElementById("cardbody").innerHTML=answer;
...
function next24F(){
    ...
    xhr.open('GET', 'https://api.openweathermap.org/data/2.5/forecast?lat='+latitude+'&lon='+longitude+'&units='+metunit+'&APPID=f79c0e875296d15f39b31adf107bc37b');
    xhr.send();
}
...
function buildnext24(t3){
    ...
    for(let i=0; i<8; i++){
        ...
        document.getElementById(te).innerHTML=t3.list[i].main.temp+tmp;
...
function buildrightNow(t2){
    ...
    document.getElementById("temp").innerHTML=t2.main.temp+tmp;
    document.getElementById("span1").innerHTML="L: "+t2.main.temp_min+tmp;
    document.getElementById("span2").innerHTML="H: "+t2.main.temp_max + tmp;
